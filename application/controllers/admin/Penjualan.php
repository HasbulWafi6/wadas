...
class Penjualan extends CI_Controller 
{
	...
	{
		$result = $this->curl->simple_get($this->API.'/penjualan');
        $conresult = json_decode($result);
        $data['penjualan'] = $conresult->penjualan;
        $this->load->view("admin/penjualan/list", $data);
	}

	public function detail($id_penjualan_m)
    {
        $result = $this->curl->simple_get($this->API.'/penjualan?id_penjualan_m='.$id_penjualan_m);
        $conresult = json_decode($result);
        //var_dump($conresult);
        $data['penjualan'] = $conresult->penjualan;
        $this->load->view('admin/penjualan/detail_form', $data);
    }
	...
	public function transaksi()
	{
					...
					if($total > 0)
					{
						...
						if($this->form_validation->run() == TRUE)
						{
							...
							else
							{
								...
								$master = $this->m_penjualan_master->insert_master($nomor_nota, $tanggal, $id_kasir, $id_perusahaan, $bayar, $grand_total, $catatan);
								if($master)
								{
			...
			else
			{
				...
				$this->load->view('admin/penjualan/transaksi', $dt);
			}
	}
	...
	public function transaksi_cetak()
	{
		...
		{
			$pelanggan = $this->m_pelanggan->get_baris($id_perusahaan)->row()->nama;
		}
	...
	public function ajax_pelanggan()
	{
		if($this->input->is_ajax_request())
		{
			...
			$data = $this->m_pelanggan->get_baris($id_perusahaan)->row();
	...
	public function history()
	{
		...
		{
			$this->load->view('admin/penjualan/transaksi_history');
		}
	}
	...
	public function history_json()
	{
		...
		{ 
			...
			$nestedData[]	= "<a href='".site_url('admin/penjualan/detail-transaksi/'.$row['id_penjualan_m'])."' id='LihatDetailTransaksi'><i class='fa fa-file-text-o fa-fw'></i> ".$row['nomor_nota']."</a>";
			...
			{
				$nestedData[]	= "<a href='".site_url('admin/penjualan/hapus-transaksi/'.$row['id_penjualan_m'])."' id='HapusTransaksi'><i class='fa fa-trash-o'></i> Hapus</a>";
			}
	...
	public function detail_transaksi($id_penjualan)
	{
		if($this->input->is_ajax_request())
		{
			...
			$this->load->view('admin/penjualan/transaksi_history_detail', $dt);
		}
	}
	...
	public function pelanggan()
	{
		...
		if($level == 'admin' OR $level == 'kasir' OR $level == 'keuangan')
		{
			$this->load->view('admin/penjualan/pelanggan_data');
		}
	}
	...
	public function pelanggan_json()
	{
		...
		{ 
			...
			if($level == 'admin' OR $level == 'kasir' OR $level == 'keuangan') 
			{
				$nestedData[]	= "<a href='".site_url('admin/penjualan/pelanggan-edit/'.$row['id_perusahaan'])."' id='EditPelanggan'><i class='fa fa-pencil'></i> Edit</a>";
			}

			if($level == 'admin') 
			{
				$nestedData[]	= "<a href='".site_url('admin/penjualan/pelanggan-hapus/'.$row['id_perusahaan'])."' id='HapusPelanggan'><i class='fa fa-trash-o'></i> Hapus</a>";
			}
		...
		if($level == 'admin' OR $level == 'kasir' OR $level == 'keuangan')
		{
			if($_POST)
			{
				...
				if($this->form_validation->run() == TRUE)
				{
					...
					if($insert)
					{
						$id_perusahaan = $this->m_pelanggan->get_dari_kode($unique)->row()->id_perusahaan;
						echo json_encode(array(
							'status' => 1,
			...
			{
				$this->load->view('admin/penjualan/pelanggan_tambah');
			}
		}
	}

	public function pelanggan_edit($id_perusahaan = NULL)
	{
		if( ! empty($id_perusahaan))
		{
			$level = $this->session->userdata('ap_level');
				...
				if($this->input->is_ajax_request())
				{
					...
					if($_POST)
					{
						...
						if($this->form_validation->run() == TRUE)
						{
							...
							$update 	= $this->m_pelanggan->update_pelanggan($id_perusahaan, $nama, $alamat, $telepon, $info);
							if($update)
							{
								echo json_encode(array(
									'status' => 1,
					...
					{
						$dt['pelanggan'] = $this->m_pelanggan->get_baris($id_perusahaan)->row();
						$this->load->view('admin/penjualan/pelanggan_edit', $dt);
					}
				}
			}
		}
	...
	public function pelanggan_hapus($id_perusahaan)
	{
		$level = $this->session->userdata('ap_level');
		if($level == 'admin')
		{
			...
			{
				$this->load->model('m_pelanggan');
				$hapus = $this->m_pelanggan->hapus_pelanggan($id_perusahaan);
				if($hapus)
				{
					echo json_encode(array(
